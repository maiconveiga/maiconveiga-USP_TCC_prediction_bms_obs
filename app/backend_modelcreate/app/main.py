...
@app.get("/")
async def root():
    logger.info("Endpoint raiz foi acessado")
    return {"message": "API Train em operação!"}

@app.get("/testar-conexao")
def endpoint_testar_conexao():
    logger.info("Endpoint teste de conexão foi acessado")
...
def train_models(request: TrainRequest):
    ...
    """
    logger.info("Endpoint de treino foi acessado")
    siteID = request.siteID
    siteName = request.siteName

    logger.info("Coletando os dados")
    df_all = getBMS()
